...
const lpSoundsProgress = {
    ...
    render(data) {
        const stageKey = `mlp_it_Stage${data.stage.charAt(0).toUpperCase()}${data.stage.slice(1)}`;
        const stageLabel = (typeof globalTranslate !== 'undefined' && globalTranslate[stageKey])
            ? globalTranslate[stageKey]
            : data.stage;
        const title = (typeof globalTranslate !== 'undefined' && globalTranslate.mlp_it_ProgressTitle)
            ? globalTranslate.mlp_it_ProgressTitle
        ...
        const warningHtml = data.stage !== 'completed'
            ? `<div class="ui small warning message" style="margin-top: 0.7em;">
                   ...
                   ${(typeof globalTranslate !== 'undefined' && globalTranslate.mlp_it_WaitBeforeSwitching) || 'Wait until conversion finishes before switching language in General Settings.'}
               </div>`
            : '';
